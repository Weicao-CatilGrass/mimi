...
- **v0.7.0 新增**：
  ...
  - ✅ Comptime 函数执行验证
- **v0.8.0 新增**（安全加固 + 标准库）：
  - ✅ 安全算术：checked_add/sub/mul/div/rem/neg/pow，整数溢出检测
  - ✅ 边界检查：数组/列表/字符串索引越界错误信息含索引和长度
  - ✅ String 操作：split, join, trim, starts_with, ends_with, replace, to_upper, to_lower, repeat, contains, index_of
  - ✅ Map 操作：map_new, map_get, map_set, map_remove, map_size, map_from_list
  - ✅ IO 操作：read_file, write_file, file_exists, input
  - ✅ 安全算术模块：safe_arith.rs（14 个 checked 函数 + 14 个单元测试）
...
> **实现状态说明**：本文档描述了 Mimi 语言的完整设计愿景。截至 v0.8.0，以下核心特性已实现并经过测试验证（813 测试通过）：
> - ✅ `cap` 线性类型与消费追踪
> - ✅ `extern "C"` 声明与解析
...
> - ✅ Comptime 函数执行验证
> - ✅ 安全算术：整数溢出检测 + 边界检查
> - ✅ 标准库：String 操作 (12 个) + Map 操作 (6 个) + IO 操作 (4 个)
> 
> 以下特性为设计目标，尚未实现：
> - ⏳ WASM 编译目标
> - ⏳ 自动绑定生成器
