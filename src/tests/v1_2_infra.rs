...
"#;
    let file = parse(src);
    let test_funcs: Vec<String> = file
        .items
        .iter()
...
fn manifest_new() {
    ...
    let pkg = manifest
        .package
        .expect("src/tests/v1_2_infra.rs:100 unwrap failed");
...
fn manifest_add_dependency() {
    ...
    manifest.add_dependency("serde", Some("1.0"), None, None, None);
    let deps = manifest
        .dependencies
        .expect("src/tests/v1_2_infra.rs:111 unwrap failed");
...
fn manifest_add_dependency_replace() {
    let mut manifest = crate::manifest::Manifest::new("test-pkg");
    manifest.add_dependency("serde", Some("1.0"), None, None, None);
    manifest.add_dependency("serde", Some("2.0"), None, None, None);
    let deps = manifest
        .dependencies
        .expect("src/tests/v1_2_infra.rs:123 unwrap failed");
    assert_eq!(deps.len(), 1);
...
fn manifest_remove_dependency() {
    let mut manifest = crate::manifest::Manifest::new("test-pkg");
    manifest.add_dependency("serde", Some("1.0"), None, None, None);
    manifest.add_dependency("tokio", Some("1.0"), None, None, None);
    assert!(manifest.remove_dependency("serde"));
    let deps = manifest
        .dependencies
        .expect("src/tests/v1_2_infra.rs:135 unwrap failed");
    assert_eq!(deps.len(), 1);
...
fn manifest_save_and_load() {
    ...
    manifest
        .save(&dir)
        .expect("src/tests/v1_2_infra.rs:154 unwrap failed");
    let loaded = crate::manifest::Manifest::load(&dir)
        .expect("src/tests/v1_2_infra.rs:155 unwrap failed")
        .expect("src/tests/v1_2_infra.rs:155 unwrap failed");
    assert_eq!(
        loaded
            .package
            .expect("src/tests/v1_2_infra.rs:156 unwrap failed")
            .name,
        "test-pkg"
    );
    let deps = loaded
        .dependencies
        .expect("src/tests/v1_2_infra.rs:157 unwrap failed");
    assert_eq!(deps.len(), 1);
    assert_eq!(deps[0].name, "serde");
...
fn lsp_completion_stdlib_functions() {
    ...
    assert!(
        labels.contains(&"replace_all"),
        "stdlib replace_all should be in completions"
    );
    assert!(
        labels.contains(&"print_line"),
        "stdlib print_line should be in completions"
    );
    assert!(
        labels.contains(&"assert_eq_int"),
        "stdlib assert_eq_int should be in completions"
    );
    assert!(
        labels.contains(&"identity"),
        "stdlib identity should be in completions"
    );
    assert!(
        labels.contains(&"parse"),
        "stdlib parse (csv) should be in completions"
    );
}
...
fn lsp_completion_stdlib_modules_context() {
    ...
    assert!(
        ...
        "strings module should appear in use context"
    );
    assert!(
        labels.contains(&"io"),
        "io module should appear in use context"
    );
    assert!(
        labels.contains(&"json"),
        "json module should appear in use context"
    );
}
