...
"#;
    ...
    let obj_path = match compile_only(src) {
        Ok(p) => p,
        Err(e) => {
            eprintln!("SKIP: compile_only failed: {}", e);
            return;
        }
    };
