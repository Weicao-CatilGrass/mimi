...
fn transitive_resolution_basic() {
    ...
    manifest.dependencies = Some(vec![crate::manifest::Dependency {
        name: "middle".into(),
    ...
    assert!(
        result.is_ok(),
        "transitive install should succeed: {:?}",
        result.err()
    );
    ...
    assert!(
        leaf_entry.is_some(),
        "lockfile should contain leaf (transitive dep)"
    );
...
fn transitive_resolution_cycle_detection() {
    ...
    assert!(
        result.is_ok(),
        "transitive install should handle cycles: {:?}",
        result.err()
    );
    ...
    assert!(
        lock.get_package("pkg-a").is_some(),
        "lockfile should contain pkg-a"
    );
    assert!(
        lock.get_package("pkg-b").is_some(),
        "lockfile should contain pkg-b"
    );
...
fn transitive_resolution_diamond() {
    ...
    assert!(
        result.is_ok(),
        "diamond install should succeed: {:?}",
        result.err()
    );
    ...
    assert!(
        lock.get_package("dep-d").is_some(),
        "lockfile should contain dep-d (shared transitive)"
    );
    assert!(
        lock.get_package("dep-b").is_some(),
        "lockfile should contain dep-b"
    );
    assert!(
        lock.get_package("dep-c").is_some(),
        "lockfile should contain dep-c"
    );
...
fn transitive_resolution_deep_chain() {
    ...
    assert!(
        result.is_ok(),
        "deep chain install should succeed: {:?}",
        result.err()
    );
...
fn manifest_add_git_dependency() {
    ...
    m.add_dependency(
        ...
        None,
        None,
        Some("https://github.com/example/my-lib"),
        Some("v1.0"),
    );
