...
fn compile_to_ir(src: &str) -> String {
    ...
    codegen
        .compile_file(&file)
        .expect("src/tests/codegen_golden.rs:8 unwrap failed");
    codegen.emit_ir()
}

/// Golden file path for a given test name.
fn golden_path(name: &str) -> std::path::PathBuf {
    let dir = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("src/tests/golden");
...
fn check_golden(name: &str, src: &str) {
    ...
    let expected =
        std::fs::read_to_string(&path).expect("src/tests/codegen_golden.rs:38 unwrap failed");
    if ir != expected {
