...
mod type_system_verification {
    ...
    fn test_extern_fn_type_in_extern() {
        let src = r#"
        extern "C" {
            func register(cb: extern "C" fn(i32) -> i32) -> i32;
        }
        func main() -> i32 { 0 }
        "#;
        assert!(parse_and_check(src).is_ok(), "extern fn should be allowed in extern");
    }
    ...
    fn test_requires_contract_parsing() {
        ...
        assert!(result.is_ok(), "requires should parse: {:?}", result.err());
    }
    ...
    fn test_passport_types() {
        let src = r#"
        extern "C" {
            fn a(x: c_shared i32) -> i32;
            fn b(x: c_borrow i32) -> i32;
            fn c(x: c_borrow_mut i32) -> i32;
            fn d(x: *i32) -> i32;
            fn e(x: *mut i32) -> i32;
        }
        func main() -> i32 { 0 }
    ...
    fn test_extern_fn_rejected_outside_extern() {
        let src = r#"
        func bad(cb: extern "C" fn(i32) -> i32) -> i32 { 0 }
        func main() -> i32 { 0 }
        "#;
