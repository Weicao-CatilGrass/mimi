...
/// Find a path to libc.so.6 at runtime, regardless of Linux distribution.
///
/// Debian/Ubuntu puts it at `/lib/x86_64-linux-gnu/libc.so.6`,
/// Arch Linux at `/usr/lib/libc.so.6`, etc.  We check well-known
/// locations and fall back to resolving via the loader.
pub(crate) fn ffi_lib_path() -> &'static str {
    ...
    PATH.get_or_init(|| {
        ...
        // Last resort: ask the dynamic linker directly.
        // `ldd` on the shell prints the resolved path of libc.
        let output = std::process::Command::new("sh")
            .args(["-c", "ldd /bin/sh | grep libc | awk '{print $3}'"])
            .output()
            .ok()
            .and_then(|o| {
                if o.status.success() {
        ...
        match output {
            ...
            None => panic!("could not locate libc.so.6 on this system"),
        }
    })
}
