...
fn test_parser_empty_input() {
    let tokens = lexer::Lexer::new("")
        .tokenize()
        .expect("src/tests/fuzz/target_parser.rs:28 unwrap failed");
    let _ = parser::Parser::new(tokens).parse_file();
}
...
fn test_parser_only_comments() {
    let tokens = lexer::Lexer::new("// just a comment\n// another\n")
        .tokenize()
        .expect("src/tests/fuzz/target_parser.rs:41 unwrap failed");
    let _ = parser::Parser::new(tokens).parse_file();
}
...
fn test_parser_extremely_long_line() {
    ...
    let tokens = lexer::Lexer::new(&long_line)
        .tokenize()
        .expect("src/tests/fuzz/target_parser.rs:65 unwrap failed");
    let _ = parser::Parser::new(tokens).parse_file();
}
...
fn test_parser_many_funcs() {
    ...
    let tokens = lexer::Lexer::new(&src)
        .tokenize()
        .expect("src/tests/fuzz/target_parser.rs:76 unwrap failed");
    let _ = parser::Parser::new(tokens).parse_file();
}
