...
fn compile_to_ir(src: &str) -> String {
    ...
    codegen
        .compile_file(&file)
        .expect("src/tests/codegen_control.rs:7 unwrap failed");
    codegen.emit_ir()
}
...
fn codegen_nested_if_else() {
    ...
    assert!(
        then_count >= 2,
        "Should have at least 2 'then' blocks for nested ifs, got {}",
        then_count
    );
}
...
fn codegen_while_break() {
    ...
    assert!(
        ir.contains("loopcont"),
        "IR should have loopcont block for break"
    );
}
...
fn codegen_function_call_chain() {
    ...
    assert!(
        def_count >= 3,
        "Should have at least 3 function definitions, got {}",
        def_count
    );
}
...
fn codegen_builtin_println_int() {
    ...
    assert!(
        ir.contains("printf"),
        "IR should contain printf for integer println"
    );
}
...
fn codegen_builtin_assert() {
    ...
    assert!(
        ir.contains("assert_fail"),
        "IR should have assert_fail block"
    );
}
...
fn codegen_builtin_len() {
    ...
    assert!(
        ir.contains("list.len"),
        "IR should load list.len for len builtin"
    );
}
...
fn codegen_builtin_to_string() {
    ...
    assert!(
        ir.contains("sprintf"),
        "IR should call sprintf for to_string"
    );
...
fn codegen_list_literal() {
    ...
    assert!(
        ir.contains("malloc"),
        "IR should call malloc for list allocation"
    );
    assert!(ir.contains("list_len"), "IR should store list length");
    assert!(
        ir.contains("list_data"),
        "IR should store list data pointer"
    );
}
...
fn codegen_list_index() {
    ...
    assert!(
        ir.contains("list.data"),
        "IR should access list.data for indexing"
    );
...
fn codegen_extern_block_no_params() {
    ...
    assert!(
        ir.contains("call"),
        "IR should contain call to extern function"
    );
}
...
fn codegen_cap_register() {
    ...
    assert!(
        ir.contains("mimi_cap_register"),
        "IR should contain cap_register call when cap literal is used"
    );
}
...
fn codegen_cap_consume() {
    ...
    assert!(
        ir.contains("mimi_cap_consume"),
        "IR should contain cap_consume call when cap is dropped"
    );
}
...
fn codegen_cap_extern_pass() {
    ...
    assert!(
        ir.contains("mimi_cap_check"),
        "IR should contain cap_check for extern param"
    );
}
...
fn codegen_cap_fail_declares_exit() {
    ...
    assert!(
        ir.contains("declare void @exit(i32)") || ir.contains("declare void @exit"),
...
fn codegen_callback_tls_cleared_after_call() {
    ...
    eprintln!(
        "IR snippets containing 'apply' or '__mimi_cb_fnptr':\n{:#?}",
        after_call
    );
}
...
fn codegen_actor_basic() {
    ...
    assert!(
        ir.contains("Counter_new"),
        "IR should contain actor constructor"
    );
...
fn codegen_actor_multiple_fields() {
    ...
    assert!(
        ir.contains("Person_new"),
        "IR should contain Person constructor"
    );
}
...
fn codegen_turbofish_generic() {
    ...
    assert!(
        ir.contains("identity$T_i32"),
        "IR should contain mangled generic function name:\n{}",
        ir
    );
}
...
fn codegen_turbofish_multiple_instantiations() {
    ...
    assert!(
        ir.contains("wrap$T_i32"),
        "IR should contain wrap$T_i32:\n{}",
        ir
    );
    assert!(
        ir.contains("wrap$T_i64"),
        "IR should contain wrap$T_i64:\n{}",
        ir
    );
}
...
fn codegen_on_failure_discarded_on_normal_exit() {
    ...
    assert!(
        !ir.contains("cleanup"),
        "OnFailure body should be discarded on normal exit:\n{}",
        ir
    );
}
...
fn codegen_on_failure_registered_not_inline() {
    ...
    assert!(
        ir.contains("compensate"),
        "OnFailure body should compile before exit:\n{}",
        ir
    );
...
fn codegen_on_failure_scope_nesting() {
    ...
    assert!(
        ir.contains("outer"),
        "Outer compensation should appear:\n{}",
        ir
    );
    assert!(
        !ir.contains("inner"),
        "Inner compensation (discarded on normal block exit) should NOT appear:\n{}",
        ir
    );
}
...
fn codegen_spawn_await_no_zero_placeholder() {
    ...
    assert!(
        ir.contains("mimi_spawn_future"),
        "IR should contain mimi_spawn_future:\n{}",
        ir
    );
...
fn codegen_stdlib_loader_roundtrip() {
    ...
    let _ = loader
        .load_main(&math_path)
        .expect("should load mymath.mimi");
...
fn codegen_type_name_known_var() {
    ...
    assert!(
        ir.contains("type_name"),
        "IR should contain the type name string:\n{}",
        ir
    );
}
...
fn e2e_hello_world() {
    ...
    let stdout = super::compile_and_run(
        r#"
        func main() -> i32 {
