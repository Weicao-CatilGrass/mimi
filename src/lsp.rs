...
impl LspServer {
        ...
        match method {
            ...
            "textDocument/completion" => {
                let uri = msg.get("params")?
                    ...
                    .get("uri")?
                    .as_str()?;
                let (line, character) = msg.get("params")
            ...
            "textDocument/prepareCallHierarchy" => {
                ...
                let character = position.get("character")?.as_u64()? as usize;
                let text = self.documents.get(uri)?;
                let items = self.compute_prepare_call_hierarchy(text, uri, line, character);
                Some(serde_json::json!({
                    "jsonrpc": "2.0",
                    "id": id,
                    "result": items
            ...
            "callHierarchy/incomingCalls" => {
                let uri = msg.get("params")?
                    ...
                    .get("uri")?
                    .as_str()?;
                let name = msg.get("params")?
                    .get("item")?
                    .get("name")?
                    .as_str()?;
                let text = self.documents.get(uri)?;
                let calls = self.compute_incoming_calls(text, uri, name);
                Some(serde_json::json!({
                    "jsonrpc": "2.0",
                    "id": id,
                    "result": calls
            ...
            "callHierarchy/outgoingCalls" => {
                ...
                let name = msg.get("params")?
                    ...
                    .as_str()?;
                let text = self.documents.get(uri)?;
                let calls = self.compute_outgoing_calls(text, uri, name);
                Some(serde_json::json!({
                    "jsonrpc": "2.0",
                    "id": id,
                    "result": calls
    ...
    pub fn compute_completion(&self, text: &str, line: usize, character: usize) -> Vec<serde_json::Value> {
    ...
    pub fn compute_prepare_call_hierarchy(&self, text: &str, uri: &str, line: usize, character: usize) -> Vec<serde_json::Value> {
        let file = match self.parse_with_recovery(text) {
            Some(f) => f,
            None => return vec![],
        };
        ...
        for item in &file.items {
            match item {
                ...
                Item::Type(t) if t.name == word => {
                    ...
                    return vec![serde_json::json!({
                        "name": t.name,
                        "kind": match t.kind { TypeDefKind::Record(_) => 23, TypeDefKind::Enum(_) => 10, _ => 4 },
                        "uri": uri,
                        "range": {
                            "start": { "line": def_line, "character": 0 },
                            "end": { "line": def_line, "character": 0 }
    ...
    pub fn compute_incoming_calls(&self, text: &str, uri: &str, name: &str) -> Vec<serde_json::Value> {
        let file = match self.parse_with_recovery(text) {
            Some(f) => f,
            None => return vec![],
        };
        ...
        for item in &file.items {
            if let Item::Func(f) = item {
                ...
                let call_lines: Vec<usize> = text.lines().enumerate()
                    .filter(|(i, l)| {
                        if Some(*i) == def_line { return false; }
                        l.contains(&format!("{}(", name))
                    })
                    .map(|(i, _)| i)
                    .collect();
                if !call_lines.is_empty() {
                    let func_line = text.lines().position(|l| l.contains(&format!("func {}", f.name))).unwrap_or(0);
    ...
    pub fn compute_outgoing_calls(&self, text: &str, uri: &str, name: &str) -> Vec<serde_json::Value> {
        let file = match self.parse_with_recovery(text) {
            Some(f) => f,
            None => return vec![],
        };
...
fn collect_calls_from_expr(
    ...
    match expr {
        Expr::Call(callee, args) => {
            if let Expr::Ident(name) = callee.as_ref() {
                if !visited.contains(name.as_str()) {
                    ...
                    let callee_line = text.lines().position(|l| l.contains(&format!("func {}", name))).unwrap_or(0);
                    let call_line = text.lines().position(|l| l.contains(&format!("{}(", name))).unwrap_or(0);
                    calls.push(serde_json::json!({
                        "to": {
                            "name": name,
                            "kind": 12,
        ...
        Expr::Unary(_, e) | Expr::Try(e) | Expr::Spawn(e) | Expr::Await(e)
            | Expr::Old(e) | Expr::QuoteInterpolate(e) | Expr::TypeOf(e) => {
            collect_calls_from_expr(e, text, items, uri, calls, visited);
        }
