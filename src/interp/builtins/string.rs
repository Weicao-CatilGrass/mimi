...
impl<'a> Interpreter<'a> {
    ...
    pub(crate) fn builtin_str_char_at(&self, args: Vec<Value>) -> Result<Value, InterpError> {
        if args.len() != 2 {
            return Err(InterpError::new(
                "str_char_at expects 2 arguments (string, index)",
            ));
        }
        match (&args[0], &args[1]) {
                ...
                s.chars()
                    ...
                    .ok_or_else(|| {
                        InterpError::new(format!(
                            "str_char_at: index {} out of bounds (len {})",
                            i,
                            s.chars().count()
                        ))
                    })
            ...
            (Value::List(parts), Value::String(sep)) => {
                ...
                for p in parts {
                    match p {
                        ...
                        _ => {
                            return Err(InterpError::new("str_join: list elements must be strings"))
                        }
                    }
                }
    ...
    pub(crate) fn builtin_str_trim(&self, args: Vec<Value>) -> Result<Value, InterpError> {
        if args.len() != 1 {
            return Err(InterpError::new("str_trim expects 1 argument"));
        }
        match &args[0] {
    ...
    pub(crate) fn builtin_str_starts_with(&self, args: Vec<Value>) -> Result<Value, InterpError> {
        if args.len() != 2 {
            return Err(InterpError::new("str_starts_with expects 2 arguments"));
        }
        match (&args[0], &args[1]) {
    ...
    pub(crate) fn builtin_str_ends_with(&self, args: Vec<Value>) -> Result<Value, InterpError> {
        if args.len() != 2 {
            return Err(InterpError::new("str_ends_with expects 2 arguments"));
        }
        match (&args[0], &args[1]) {
    ...
    pub(crate) fn builtin_str_replace(&self, args: Vec<Value>) -> Result<Value, InterpError> {
        if args.len() != 3 {
            return Err(InterpError::new("str_replace expects 3 arguments"));
        }
        match (&args[0], &args[1], &args[2]) {
            ...
            _ => Err(InterpError::new(
                "str_replace expects (string, string, string)",
            )),
        }
    }
    ...
    pub(crate) fn builtin_str_to_upper(&self, args: Vec<Value>) -> Result<Value, InterpError> {
        if args.len() != 1 {
            return Err(InterpError::new("str_to_upper expects 1 argument"));
        }
        match &args[0] {
    ...
    pub(crate) fn builtin_str_to_lower(&self, args: Vec<Value>) -> Result<Value, InterpError> {
        if args.len() != 1 {
            return Err(InterpError::new("str_to_lower expects 1 argument"));
        }
        match &args[0] {
    ...
    pub(crate) fn builtin_str_repeat(&self, args: Vec<Value>) -> Result<Value, InterpError> {
        if args.len() != 2 {
            return Err(InterpError::new("str_repeat expects 2 arguments"));
        }
        match (&args[0], &args[1]) {
            (Value::String(s), Value::Int(n)) => {
                if *n < 0 {
                    return Err(InterpError::new("str_repeat: count must be non-negative"));
                }
        ...
        match (&args[0], &args[1]) {
            (Value::String(s), Value::String(pattern)) => {
                let re = regex::Regex::new(pattern).map_err(|e| {
                    InterpError::new(format!("regex_match: invalid pattern '{}': {}", pattern, e))
                })?;
    ...
    pub(crate) fn builtin_regex_find(&self, args: Vec<Value>) -> Result<Value, InterpError> {
        ...
        match (&args[0], &args[1]) {
            (Value::String(s), Value::String(pattern)) => {
                let re = regex::Regex::new(pattern).map_err(|e| {
                    InterpError::new(format!("regex_find: invalid pattern '{}': {}", pattern, e))
                })?;
                match re.find(s) {
        ...
        match (&args[0], &args[1]) {
            (Value::String(s), Value::Int(idx)) => {
                ...
                s.chars()
                    ...
                    .ok_or_else(|| {
                        InterpError::new(format!(
                            "char_code: index {} out of bounds (len {})",
                            i,
                            s.chars().count()
                        ))
                    })
    ...
    pub(crate) fn builtin_chr(&self, args: Vec<Value>) -> Result<Value, InterpError> {
        if args.len() != 1 {
            return Err(InterpError::new("chr expects 1 argument (code point)"));
        }
        match &args[0] {
            Value::Int(code) => {
                if *code < 0 || *code > 0x10FFFF {
                    return Err(InterpError::new(format!(
                        "chr: code point {} out of range (0-0x10FFFF)",
                        code
                    )));
                }
                match char::from_u32(*code as u32) {
                    ...
                    None => Err(InterpError::new(format!(
                        "chr: invalid Unicode code point {}",
                        code
                    ))),
                }
            }
