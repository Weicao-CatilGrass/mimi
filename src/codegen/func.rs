    ...
    pub(super) fn compile_async_func(&mut self, func: &FuncDef) -> MimiResult<()> {
        ...
        for (param_idx, _param) in func.params.iter().enumerate() {
            if param_idx < param_types.len() {
                ...
                let arg_val = self
                    .builder
                    .build_load(ty, arg_typed_ptr, &format!("poll_arg_val_{}", param_idx))
            ...
            .map_or(true, |t| matches!(t, Type::Name(n, _) if n == "unit"))
        {
            ...
            self.builder
                .build_store(result_typed_ptr, poll_body_result)
        ...
        for (param_idx, param) in func.params.iter().enumerate() {
            if param_idx < param_types.len() {
                ...
                let val = self
                    .builder
                    .build_load(ty, alloca.0, &format!("store_{}", param.name))
                ...
                self.builder
                    .build_store(arg_slot_typed, val)
        ...
        self.builder
            .build_return(Some(&BasicValueEnum::PointerValue(future_ptr)))
        ...
        for (i, param) in func.params.iter().enumerate() {
            if let Some(ty) = self.llvm_type_for(&param.ty) {
                ...
                if let Type::DynTrait(_) = &param.ty {
                    self.var_type_names
                        .insert(param.name.clone(), crate::core::fmt_type(&param.ty));
                }
        ...
        let last_val = match (last_val, ret_type) {
            (BasicValueEnum::PointerValue(pv), BasicTypeEnum::StructType(st)) => {
                ...
                if is_string_struct {
                    ...
                    let loaded = self
                        .builder
                        .build_load(BasicTypeEnum::StructType(st), pv, "ret_load")
                        .map_err(|e| {
                            CompileError::LlvmError(format!("load return struct: {}", e))
                        })?;
        ...
        for (i, param) in func.params.iter().enumerate() {
            ...
            if let Some(ty) = self.llvm_type_for(&resolved) {
                ...
                if let Type::DynTrait(_) = &resolved_param {
                    self.var_type_names
                        .insert(param.name.clone(), crate::core::fmt_type(&resolved_param));
                }
