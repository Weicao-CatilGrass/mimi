...
impl<'ctx> CodeGenerator<'ctx> {
    ...
    pub(in crate::codegen) fn compile_list_expr(
        ...
        for (i, elem) in elems.iter().enumerate() {
            ...
            let iv = match val {
                ...
                BasicValueEnum::FloatValue(fv) => self
                    .builder
                    .build_bit_cast(fv, self.context.i64_type(), "f64_to_i64")
                    .map_err(|e| CompileError::LlvmError(format!("bitcast error: {}", e)))?
                    .into_int_value(),
        ...
        let data_void_ptr = self
            ...
            .map_err(|e| CompileError::LlvmError(format!("bitcast error: {}", e)))?;
        self.builder
            .build_store(data_gep, data_void_ptr)
            .map_err(|e| CompileError::LlvmError(format!("store error: {}", e)))?;
    ...
    pub(in crate::codegen) fn compile_comprehension_expr(
        ...
        let wi_alloca = self
            .builder
            .build_alloca(i64_ty, "cw")
            .map_err(|e| CompileError::LlvmError(format!("alloca error: {}", e)))?;
        self.builder
            .build_store(idx_alloca, i64_ty.const_int(0, false))
            .map_err(|e| CompileError::LlvmError(format!("store error: {}", e)))?;
        self.builder
            .build_store(wi_alloca, i64_ty.const_int(0, false))
            .map_err(|e| CompileError::LlvmError(format!("store error: {}", e)))?;
        self.builder
        ...
        self.builder
            .build_conditional_branch(include, store_bb, next_bb)
            .map_err(|e| CompileError::LlvmError(format!("branch error: {}", e)))?;
        ...
        let result_i64 = match result {
            ...
            BasicValueEnum::PointerValue(pv) => self
                .builder
                .build_ptr_to_int(pv, i64_ty, "p_to_i")
        ...
        self.builder
            .build_store(out_elem_ptr, result_i64)
            .map_err(|e| CompileError::LlvmError(format!("store error: {}", e)))?;
        let next_wi = self
            .builder
        ...
        let result_len = self
            .builder
            .build_load(BasicTypeEnum::IntType(i64_ty), wi_alloca, "result_len")
            .map_err(|e| CompileError::LlvmError(format!("load error: {}", e)))?;
        let result_alloca = self
            .builder
            .build_alloca(list_struct_ty, "comp_result")
            .map_err(|e| CompileError::LlvmError(format!("alloca error: {}", e)))?;
        let rlen_gep = self
            .gep()
            ...
            .map_err(|e| CompileError::LlvmError(format!("gep error: {}", e)))?;
        self.builder
            .build_store(rlen_gep, result_len)
            .map_err(|e| CompileError::LlvmError(format!("store error: {}", e)))?;
        let rdata_gep = self
            .gep()
        ...
        let out_void = self
            ...
            .map_err(|e| CompileError::LlvmError(format!("bitcast error: {}", e)))?;
        self.builder
            .build_store(rdata_gep, out_void)
            .map_err(|e| CompileError::LlvmError(format!("store error: {}", e)))?;
