...
impl<'ctx> CodeGenerator<'ctx> {
    pub(in crate::codegen) fn build_string_list(
        ...
        let arr_alloca = self
            .builder
            .build_alloca(BasicTypeEnum::ArrayType(arr_type), "str_arr")
        ...
        for (i, s) in strings.iter().enumerate() {
            ...
            let len_gep = self
                ...
                .map_err(|e| CompileError::LlvmError(format!("gep error: {}", e)))?;
            self.builder
                .build_store(len_gep, i64_ty.const_int(s.len() as u64, false))
                .map_err(|e| CompileError::LlvmError(format!("store error: {}", e)))?;
        }
        ...
        let len_gep = self
            ...
            .map_err(|e| CompileError::LlvmError(format!("gep error: {}", e)))?;
        self.builder
            .build_store(len_gep, i64_ty.const_int(count, false))
            .map_err(|e| CompileError::LlvmError(format!("store error: {}", e)))?;
        let data_gep = self
            .gep()
            .build_struct_gep(list_ty, list_alloca, 1, "data")
            .map_err(|e| CompileError::LlvmError(format!("gep error: {}", e)))?;
        let arr_void_ptr = self
            .builder
            .build_pointer_cast(arr_alloca, i8_ptr, "arr_void")
            .map_err(|e| CompileError::LlvmError(format!("bitcast error: {}", e)))?;
        self.builder
            .build_store(data_gep, arr_void_ptr)
            .map_err(|e| CompileError::LlvmError(format!("store error: {}", e)))?;
        Ok(list_alloca.into())
    }
    ...
    pub(in crate::codegen) fn compile_builtin_intrinsic(
        ...
        match name {
                    ...
                    if name == "keys" {
                        ...
                        if let Some(BasicTypeEnum::StructType(_struct_ty)) = llvm_ty {
                            ...
                            let alloc_size = self
                                ...
                                .map_err(|e| {
                                    CompileError::LlvmError(format!("mul error: {}", e))
                                })?;
                            let malloc_fn = self
                                .module
                                .get_function("malloc")
                            ...
                            if let TypeDefKind::Record(fields) = &type_def.kind {
                                for (i, field) in fields.iter().enumerate() {
                                    ...
                                    let field_ty =
                                        types::mimi_type_to_llvm(self.context, &field.ty)
                                            .unwrap_or(BasicTypeEnum::IntType(i64_ty));
                                    let val = self
                                        .builder
                                        .build_load(field_ty, gep, &field.name)
                                        .map_err(|e| {
                                    ...
                                    self.builder.build_store(elem_ptr, val_i64).map_err(|e| {
                                ...
                                let result_alloca = self
                                    .builder
                                    .build_alloca(result_list_ty, "values_result")
                                    .map_err(|e| {
                                ...
                                self.builder
                                    .build_store(result_data_gep, values_data_void)
                                    .map_err(|e| {
                                        CompileError::LlvmError(format!("store error: {}", e))
                                    })?;
            ...
            "map" | "filter" if args.len() == 2 => {
                ...
                let result_alloca = self
                    .builder
                    .build_alloca(result_ty, "map_result")
                    .map_err(|e| CompileError::LlvmError(format!("alloca error: {}", e)))?;
                ...
                let write_idx = self
                    .builder
                    .build_alloca(i64_ty, "wi")
                    .map_err(|e| CompileError::LlvmError(format!("alloca error: {}", e)))?;
                self.builder
                    .build_store(idx_alloca, i64_ty.const_int(0, false))
                    .map_err(|e| CompileError::LlvmError(format!("store error: {}", e)))?;
                self.builder
                    .build_store(write_idx, i64_ty.const_int(0, false))
                    .map_err(|e| CompileError::LlvmError(format!("store error: {}", e)))?;
                self.builder
                ...
                self.builder
                    .build_conditional_branch(loop_cmp, body_bb, done_bb)
                    .map_err(|e| CompileError::LlvmError(format!("branch error: {}", e)))?;
                ...
                if is_map {
                    ...
                    self.builder
                        .build_conditional_branch(truthy, store_bb, next_bb)
                        .map_err(|e| CompileError::LlvmError(format!("branch error: {}", e)))?;
                ...
                let out_len = if is_map {
                    ...
                    self.builder
                        .build_load(BasicTypeEnum::IntType(i64_ty), write_idx, "out_len")
                        .map_err(|e| CompileError::LlvmError(format!("load error: {}", e)))?
                };
                let out_len_gep = self
                    ...
                    .map_err(|e| CompileError::LlvmError(format!("gep error: {}", e)))?;
                self.builder
                    .build_store(out_len_gep, out_len)
                    .map_err(|e| CompileError::LlvmError(format!("store error: {}", e)))?;
                ...
                let out_void = self
                    .builder
                    .build_pointer_cast(out_i64, i8_ptr, "out_void")
                    .map_err(|e| CompileError::LlvmError(format!("bitcast error: {}", e)))?;
                self.builder
                    .build_store(out_data_gep, out_void)
                    .map_err(|e| CompileError::LlvmError(format!("store error: {}", e)))?;
            ...
            "reduce" if args.len() == 3 => {
                ...
                let acc = self
                    .builder
                    .build_load(BasicTypeEnum::IntType(i64_ty), acc_alloca, "acc")
                    .map_err(|e| CompileError::LlvmError(format!("load error: {}", e)))?;
                let fn_result = self
                    .builder
                    .build_call(
                ...
                self.builder
                    .build_store(acc_alloca, fn_result)
                    .map_err(|e| CompileError::LlvmError(format!("store error: {}", e)))?;
                let next = self
                    .builder
                    .build_int_add(idx, i64_ty.const_int(1, false), "next")
                ...
                let result = self
                    .builder
                    .build_load(BasicTypeEnum::IntType(i64_ty), acc_alloca, "result")
                    .map_err(|e| CompileError::LlvmError(format!("load error: {}", e)))?;
                Ok(result)
            }
