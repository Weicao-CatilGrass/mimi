        ...
        match pat {
            ...
            Pattern::Literal(lit) => {
                let lit_val = self
                    ...
                    .map_err(|e| {
                        CompileError::LlvmError(format!("literal pattern compile error: {}", e))
                    })?;
                let eq = match (&val, &lit_val) {
                    (BasicValueEnum::IntValue(a), BasicValueEnum::IntValue(b)) => self
                        .builder
                        .build_int_compare(inkwell::IntPredicate::EQ, *a, *b, "pat_lit_eq")
                ...
                let assert_fn = self
                    .module
                    .get_function("mimi_runtime_assert")
                    .unwrap_or_else(|| {
                ...
                let msg = self
                    .builder
                    .build_global_string_ptr("pattern literal match failed", "pat_lit_msg")
            ...
            Pattern::Constructor(_name, sub_patterns) => {
                ...
                let struct_val = match val {
                    BasicValueEnum::PointerValue(pv) => {
                        ...
                        let loaded = self
                            .builder
                            .build_load(struct_ty, pv, "ctor_loaded")
                ...
                if sub_patterns.len() == 1 {
                    let payload = self
                        .builder
                        .build_extract_value(struct_val, 1, "ctor_payload")
                        .map_err(|e| {
                    ...
                    BasicValueEnum::PointerValue(pv) => {
                        ...
                        let struct_ty = self.tuple_type_stack.last().ok_or_else(|| {
                            CompileError::LlvmError(
                                "tuple_type_stack empty for tuple pattern".to_string(),
                            )
                        })?;
                        let loaded = self
                            .builder
        ...
        match pat {
            ...
            Pattern::Literal(lit) => {
                ...
                match (val, &lit_val) {
                    (BasicValueEnum::IntValue(a), BasicValueEnum::IntValue(b)) => self
                        .builder
                        .build_int_compare(inkwell::IntPredicate::EQ, *a, *b, "pat_lit_check")
                        .map_err(|e| CompileError::LlvmError(format!("icmp: {}", e))),
                    _ => Err(CompileError::LlvmError(
                        "literal pattern check: type mismatch".to_string(),
                    )),
                }
            }
            Pattern::Constructor(name, _) => {
                ...
                let tag = self
                    .builder
                    .build_extract_value(val_sv, 0, "enum_tag")
            ...
            Pattern::Tuple(sub_pats) => {
                ...
                for (i, sub_pat) in sub_pats.iter().enumerate() {
                    ...
                    result = self
                        .builder
                        .build_and(result, sub_match, "pat_tuple_and")
    ...
    fn extract_struct_field(
        ...
        match val {
            BasicValueEnum::StructValue(sv) => self
                .builder
                .build_extract_value(*sv, index as u32, &format!("field_{}", index))
