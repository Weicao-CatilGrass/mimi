...
impl<'ctx> CodeGenerator<'ctx> {
    ...
    pub(in crate::codegen) fn get_or_create_callback_thunk(
        ...
        let fn_ptr_val = self
            .builder
            .build_load(i8_ptr, fn_ptr_global.as_pointer_value(), "tls_fn_ptr")
            .map_err(|e| CompileError::LlvmError(format!("load fn_ptr: {}", e)))?;
        let env_ptr_val = self
            .builder
            .build_load(i8_ptr, env_ptr_global.as_pointer_value(), "tls_env_ptr")
    ...
    pub(in crate::codegen) fn generate_extern_fn(&mut self, name: &str) -> MimiResult<()> {
        ...
        let extern_fn_type = if is_void_return {
            self.context
                .void_type()
                .fn_type(&extern_param_tys, is_variadic)
        } else {
            match extern_ret_ty {
                ...
                if let Some(check_fn) = self.module.get_function("mimi_cap_check") {
                    ...
                    let function = self.current_function().ok_or_else(|| {
                        CompileError::LlvmError(
                            "codegen: no current function for cap check in extern block"
                                .to_string(),
                        )
                    })?;
                    let ok_bb = self
                        .context
                        .append_basic_block(function, &format!("cap_ok_{}", i));
                    let fail_bb = self
                        .context
                        .append_basic_block(function, &format!("cap_fail_{}", i));
                    self.builder
            ...
            match &p.ty {
                crate::ast::Type::Name(n, _) if n == "string" => {
                    ...
                    let ptr = self
                        ...
                        .map_err(|e| {
                            CompileError::LlvmError(format!("extract value error: {}", e))
                        })?;
                            ...
                            && !self.repr_c_record_names.contains(n.as_str())) =>
                {
                    ...
                    let len_val = self
                        .builder
                        .build_load(self.context.i64_type(), len_gep, &format!("list_len_{}", i))
                    ...
                    let serialize_fn =
                        if let Some(f) = self.module.get_function("mimi_json_serialize") {
                            f
                        } else {
                    ...
                    let json_val = self
                        ...
                        .map_err(|e| {
                            CompileError::LlvmError(format!("json serialize error: {}", e))
                        })?
                        .try_as_basic_value_opt()
                        .ok_or_else(|| {
                            CompileError::LlvmError("json serialize returned void".to_string())
                        })?
                        .into_pointer_value();
                ...
                crate::ast::Type::Tuple(elems) => {
                    ...
                    for (ei, elem_ty) in elems.iter().enumerate() {
                        ...
                        let elem_i64 = match elem_raw {
                            ...
                            BasicValueEnum::FloatValue(fv) => {
                                ...
                                let f_alloca = self
                                    ...
                                    .map_err(|e| {
                                        CompileError::LlvmError(format!("alloca: {}", e))
                                    })?;
                                self.builder.build_store(f_alloca, fv).map_err(|e| {
                            ...
                            BasicValueEnum::StructValue(sv) => {
                                ...
                                self.builder.build_store(s_alloca, sv).map_err(|e| {
                                    CompileError::LlvmError(format!("store: {}", e))
                                })?;
                                let cast = self
                                    .builder
                        ...
                        self.builder
                            .build_store(ty_gep, tag_i64)
                            .map_err(|e| CompileError::LlvmError(format!("store: {}", e)))?;
                    }
                    let n_i64 = i64_ty.const_int(n as u64, false);
                    let serialize_fn = if let Some(f) =
                        self.module.get_function("mimi_tuple_serialize")
                    {
                        f
                    } else {
                    ...
                    let json_val = self
                        ...
                        .try_as_basic_value_opt()
                        .ok_or_else(|| {
                            CompileError::LlvmError("tuple serialize returned void".to_string())
                        })?
                        .into_pointer_value();
                    json_strings.push(json_val);
                    wrapper_args.push(BasicMetadataValueEnum::PointerValue(json_val));
                                ...
                                if let Some(td) = self.type_defs.get(n.as_str()) {
                                    if let crate::ast::TypeDefKind::Record(fields) = &td.kind {
                                        if is_simple_reprc_record(fields) {
                                            ...
                                            wrapper_args
                                                .push(BasicMetadataValueEnum::IntValue(packed));
                                            continue;
                                        } else {
                                            ...
                                            let c_struct_ty =
                                                self.context.struct_type(&c_field_tys, false);
                                            let c_alloca = self
                                                .builder
                                                .build_alloca(
                                            ...
                                            for (fi, f) in fields.iter().enumerate() {
                                                ...
                                                self.builder.build_store(gep, c_val).map_err(
                                                    |e| {
                                                        CompileError::LlvmError(format!(
                                                            "store: {}",
                                                            e
                                            ...
                                            wrapper_args
                                                .push(BasicMetadataValueEnum::PointerValue(c_ptr));
                                            continue;
                                        }
                                    }
                                }
        ...
        if ef.no_panic {
            ...
            self.builder
                .build_call(install_fn, &[], "no_panic_install")
        ...
        let call = self
            ...
            .map_err(|e| {
                CompileError::LlvmError(format!("failed to build extern wrapper call: {}", e))
            })?;
        ...
        if ef.no_panic {
            ...
            self.builder
                .build_call(restore_fn, &[], "no_panic_restore")
        ...
        if self.verify_contracts {
            if let Some(ens_expr) = &ef.ensures {
                ...
                let result_alloca = self
                    .builder
                    .build_alloca(result_ty, "result")
                    .map_err(|e| CompileError::LlvmError(format!("alloca error: {}", e)))?;
                self.builder
                    .build_store(result_alloca, ret_val)
                    .map_err(|e| CompileError::LlvmError(format!("store error: {}", e)))?;
        ...
        if needs_json_deserialize {
            ...
            if is_tuple_return {
                ...
                let tys_alloca = self
                    .builder
                    .build_alloca(BasicTypeEnum::ArrayType(arr_ty), "tuple_ret_tys")
                    .map_err(|e| CompileError::LlvmError(format!("alloca: {}", e)))?;
                for (ei, elem_ty) in elems.iter().enumerate() {
                    let tag_val = elem_type_tag(elem_ty);
                    let tag_i64 = i64_ty.const_int(tag_val as u64, false);
                ...
                let out_alloca = self
                    .builder
                    .build_alloca(BasicTypeEnum::ArrayType(arr_ty), "tuple_ret_vals")
                    .map_err(|e| CompileError::LlvmError(format!("alloca: {}", e)))?;
                ...
                let struct_alloca = self
                    .builder
                    .build_alloca(struct_ty, "tuple_ret")
                    .map_err(|e| CompileError::LlvmError(format!("alloca: {}", e)))?;
                for (ei, elem_ty) in elems.iter().enumerate() {
                    let idx = i32_ty.const_int(ei as u64, false);
                    ...
                    let field_val: BasicValueEnum = match elem_llvm_ty {
                        BasicTypeEnum::FloatType(ft) => {
                            ...
                            let tmp_alloca = self
                                ...
                                .map_err(|e| CompileError::LlvmError(format!("alloca: {}", e)))?;
                            self.builder
                                .build_store(tmp_alloca, raw_i64)
                                .map_err(|e| CompileError::LlvmError(format!("store: {}", e)))?;
                            let cast = self
                                .builder
                                .build_pointer_cast(
                        ...
                        BasicTypeEnum::PointerType(_pt) => {
                            ...
                            let int_to_ptr = self
                                ...
                                .map_err(|e| {
                                    CompileError::LlvmError(format!("int to ptr: {}", e))
                                })?;
                ...
                let ret_val = self
                    .builder
                    .build_load(struct_ty, struct_alloca, "tuple_ret_val")
                    .map_err(|e| CompileError::LlvmError(format!("load: {}", e)))?;
                self.builder
                    .build_return(Some(&ret_val))
                ...
                let out_len_alloca = self
                    .builder
                    .build_alloca(self.context.i64_type(), "out_len")
                    .map_err(|e| CompileError::LlvmError(format!("alloca error: {}", e)))?;
                ...
                let deserialize_fn =
                    if let Some(f) = self.module.get_function("mimi_json_deserialize") {
                        f
                    } else {
                        let fn_ty = i8_ptr_ty.fn_type(
                            &[
                ...
                let len_val = self
                    .builder
                    .build_load(self.context.i64_type(), out_len_alloca, "list_len")
                    .map_err(|e| CompileError::LlvmError(format!("load error: {}", e)))?;
                if let Some(free_fn) = self.module.get_function("free") {
                    self.builder
                        .build_call(
                ...
                let list_alloca = self
                    .builder
                    .build_alloca(list_struct_ty, "list_ret")
                    .map_err(|e| CompileError::LlvmError(format!("alloca error: {}", e)))?;
                let len_gep = self
                    .gep()
                    .build_struct_gep(list_struct_ty, list_alloca, 0, "list_len_gep")
                    .map_err(|e| CompileError::LlvmError(format!("gep error: {}", e)))?;
                self.builder
                    .build_store(len_gep, len_val)
                    .map_err(|e| CompileError::LlvmError(format!("store error: {}", e)))?;
                let data_gep = self
                    .gep()
                    .build_struct_gep(list_struct_ty, list_alloca, 1, "list_data_gep")
                    .map_err(|e| CompileError::LlvmError(format!("gep error: {}", e)))?;
                self.builder
                    .build_store(data_gep, data_ptr_val)
                    .map_err(|e| CompileError::LlvmError(format!("store error: {}", e)))?;
                let ret_val = self
                    .builder
                    .build_load(list_struct_ty, list_alloca, "list_ret_val")
                    .map_err(|e| CompileError::LlvmError(format!("load error: {}", e)))?;
                self.builder
                    .build_return(Some(&ret_val))
            ...
            .is_some_and(|t| matches!(t, crate::ast::Type::Name(n, _) if n == "string"))
        {
            ...
            let alloca = self
                .builder
                .build_alloca(struct_ty, "string_ret")
                .map_err(|e| CompileError::LlvmError(format!("alloca: {}", e)))?;
            let ptr_gep = self
                .gep()
            ...
            let ret_val = self
                .builder
                .build_load(struct_ty, alloca, "string_ret_val")
                .map_err(|e| CompileError::LlvmError(format!("load: {}", e)))?;
            self.builder
                .build_return(Some(&ret_val))
                .map_err(|e| CompileError::LlvmError(format!("return: {}", e)))?;
            ...
            self.builder.build_return(Some(&ret)).map_err(|e| {
                CompileError::LlvmError(format!("failed to build extern wrapper return: {}", e))
            })?;
        } else {
