...
impl<'ctx> CodeGenerator<'ctx> {
    pub(in crate::codegen) fn compile_range(
        ...
        let function = self
            .current_function()
            .ok_or_else(|| "codegen: no current function for range loop".to_string())?;
        ...
        let elem_val = self
            .builder
            .build_int_add(start, idx, "elem_val")
            .map_err(|e| CompileError::LlvmError(format!("add error: {}", e)))?;
        let elem_ptr = {
            self.gep()
                .build_in_bounds_gep(i64_ty, data_ptr_i64, &[idx], "elem_ptr")
        }
