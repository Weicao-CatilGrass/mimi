...
impl<'ctx> CodeGenerator<'ctx> {
    pub(super) fn compile_str_char_at(
        ...
        let str_alloca = self
            .builder
            .build_alloca(string_ty, "char_str")
    ...
    pub(super) fn compile_chr(
        ...
        let str_alloca = self
            .builder
            .build_alloca(string_ty, "chr_str")
            .map_err(|e| CompileError::LlvmError(format!("alloca error: {}", e)))?;
        let ptr_gep = self
            .gep()
            .build_struct_gep(string_ty, str_alloca, 0, "str_ptr")
    ...
    pub(super) fn compile_str_parse_int(
        ...
        if args.len() != 1 {
            return Err(CompileError::WrongArgCount(
                "str_parse_int/to_int expects 1 argument".to_string(),
            ));
        }
        let i64_ty = self.context.i64_type();
    ...
    pub(super) fn compile_str_parse_float(
        ...
        if args.len() != 1 {
            return Err(CompileError::WrongArgCount(
                "str_parse_float/to_float expects 1 argument".to_string(),
            ));
        }
